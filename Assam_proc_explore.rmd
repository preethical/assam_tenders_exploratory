...
assam_tenders_total <- assam_published %>% group_by(publishedyear) %>% tally()
assam_tend_cat <- assam_published %>% group_by(publishedyear, tender_category) %>% tally()

assam_org<- assam_published %>% group_by(Org, publishedyear) %>% summarise(total = sum(value_of_tender_in_rs, na.rm = TRUE))

assam_tenders_oc <- assam_oc %>%  group_by(publishedyear > 2018) %>% summarise(Unique_Elements = n_distinct(name_of_awarded_bidders))
assam_org <- assam_org %>% 
arrange(desc(total)) %>% 
...
assam_time <- assam_published %>% group_by(publishedyear, day_category) %>% tally()
...
ggplotly(assam_time %>% ggplot(aes(x= publishedyear, y = n, fill = day_category)) + 
...
```{r initiallook6, echo=FALSE, fig.height=10, fig.width=14, message=FALSE, warning=FALSE}
assam_tenders_cancelled <- assam_published %>% group_by(tender_status, publishedyear) %>% tally()



assam_tenders_aoc <- assam_tenders_aoc %>% separate(organisation_chain, into = c("Org","role"), sep = "\\|\\|", extra = "merge")
assam_bids <- assam_oc %>% group_by(Org,no_of_bids_received) %>% summarise(total = sum(awarded_price_in_rs, na.rm = TRUE))

assam_bid_attempt <- assam_oc %>% group_by(no_of_bids_received == 1, publishedyear > 2018) %>% tally()
