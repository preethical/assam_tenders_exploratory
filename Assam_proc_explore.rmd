...
```
- We then see the numbers of tenders under different tendering stages across organizations and through the years. 
...
```
Look to see what the distribution of tenders across organizations and tender status/stages are. 
```{r initiallook3, echo = TRUE}
assam_status <- assam_published %>% group_by(Org,tender_status, publishedyear) %>%  tally ()

assam_status<- assam_status %>% 
arrange(desc(n)) %>% 
group_by(publishedyear, tender_status) %>% slice(1:5)

ggplotly(ggplot(data = assam_status, aes(x = reorder(Org, -n), y=n, fill = tender_status)) + geom_bar(stat = "identity") + facet_grid(~publishedyear, scales = "free") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=.5,colour='black')))

assam_published$stage_status_updated_on<- as.Date(assam_published$stage_status_updated_on, 
                                                        format="%d-%b-%Y")

assam_published$updatedyear<- year(assam_published$stage_status_updated_on)

assam_status_year <- assam_published %>% group_by(Org,tender_status, updatedyear) %>%  tally ()

assam_status_year<- assam_status_year %>% 
arrange(desc(n)) %>% 
group_by(updatedyear, tender_status) %>% slice(1:5)

ggplotly(ggplot(data = assam_status_year, aes(x = reorder(Org, -n), y=n, fill = factor(updatedyear))) + geom_bar(stat = "identity") + facet_grid(~tender_status, scales = "free") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=.5,colour='black')))
