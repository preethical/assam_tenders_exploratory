...
assam_list$Name <- toupper(assam_list$Name)
assam_list$District <- toupper(assam_list$District)

for(i in seq_len(nrow(assam_list))) {
  want <- grep(assam_list[i, "Name"], assam_published[,"tender_title"],ignore.case = T)
  assam_published[want, "Place"] <- assam_list[i, "Name"]
}  
for(i in seq_len(nrow(assam_list))) {
  want <- grep(assam_list[i, "Name"], assam_published[,"Org"],ignore.case = T)
  assam_published[want, "Place1"] <- assam_list[i, "Name"]
}  

for(i in seq_len(nrow(assam_list))) {
  want <- grep(assam_list[i, "Name"], assam_published[,"tender_reference_no"],ignore.case = T)
  assam_published[want, "Place2"] <- assam_list[i, "Name"]
}  

newtable <- assam_published %>% filter(is.na(Place))
newtable_1 <- newtable %>% filter(is.na(Place1))
newtable_2 <- newtable_1 %>% filter(is.na(Place2))
